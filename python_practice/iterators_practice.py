...
- метод Sentence()._words должен возвращать ленивый итератор. Cмысл тут в том, что мы не хотим хранить в объекте список слов, потому что предложения могут быть очень большими и занимать много памяти, по этому мы будем генерировать его по необходимости и отдавать пользователю
пример работы: Sentence('Hello word!')._words() >>> <generator object Sentence._words at 0x7f4e8cb065f0> next(Sentence('Hello word!')._words()) >>> 'Hello'
- имеет свойство Sentence().words, которое возвращает список всех слов в предложении (*напоминаю, что мы не хотим хранить все эти слова в нашем объекте)
- имеет свойство Sentence().other_chars, которое возвращает список всех не слов в предложении
...
- при передаче в качестве аргумента в функцию next() по очереди возвращает слова из порождающего его объекта Sentence"""
...
class Sentence():
    ...
    @property
    def words(self):
        return [word for word in SentenceIterator(self.sentence)]
    ...
    def find_word(self, start_char):
        ...
        for i in zipp:
            ...
            if re.search(pattern_letter, i[0]) and re.search(pattern_nonletter, i[1]):
                break
            end_word += 1
        return word, end_word + start_char
